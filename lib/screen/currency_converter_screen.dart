...
class _CurrencyConverterScreenState extends State<CurrencyConverterScreen> {
  ...
  Future<void> _fetchSupportedCurrencies() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final Map<String, String> parsedMap = jsonData.map((key, value) => MapEntry(key, value.toString()));
        setState(() {
          _currencyMap = parsedMap;
          _currencyCodes = parsedMap.keys.toList()..sort();
          _fromCurrency = 'USD';
          _toCurrency = _currencyCodes.contains('EUR') ? 'EUR' : _currencyCodes.first;
        });
        await _convertCurrency(); // ✅ Trigger default conversion
      }
    } catch (e) {
  ...
  Future<void> _convertCurrency() async {
    ...
    try {
      final uri = Uri.parse(
        "https://api.frankfurter.dev/latest?amount=$amount&base=$_fromCurrency&symbols=$_toCurrency"
      );
      ...
      if (response.statusCode == 200) {
        ...
        final result = "$amount $_fromCurrency = ${rate.toStringAsFixed(2)} $_toCurrency";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Padding(
        ...
        child: _isLoading && _currencyMap.isEmpty
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: const Text("Convert Now", style: TextStyle(fontSize: 16)),
                  ...
                  // ✅ CONVERSION DISPLAY (no animation, full width, centered)
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_convertedRate != null)
                              Text(
                                "1 $_fromCurrency = ${_convertedRate!.toStringAsFixed(4)} $_toCurrency",
                  ...
                  // ✅ BEAUTIFIED HISTORY
                  Text("Conversion History", style: Theme.of(context).textTheme.titleMedium),
                  const SizedBox(height: 8),
                  Expanded(
                    child: _conversionHistory.isEmpty
                        ? const Center(child: Text("No conversions yet.", style: TextStyle(color: Colors.grey)))
                        : ListView.separated(
    ...
    return DropdownButtonFormField<String>(
      ...
      items: _currencyCodes
          .map((code) => DropdownMenuItem(
                value: code,
                child: Text("$code | ${_currencyMap[code]}", style: const TextStyle(fontSize: 16)),
              ))
          .toList(),
      onChanged: onChanged,
      decoration: InputDecoration(
